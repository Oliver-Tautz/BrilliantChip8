...
protected:
    ...
    void stepAndCheck(uint8_t stepNum, std::function<void(const BrilliantChip8::Chip8StateSnapshot &)> checker)
    {
        chip.emulateCycle();
...
TEST_F(BrilliantChip8OpcodeStepTest, ExecutesWithSubroutineAndDrawsFont)
{
    ...
    EXPECT_EQ(s6.program_counter, 0x202); // should return to instruction after CALL
}
