...
void OpCodeExecutor::OPCODE_8XY5_SubtractVYFromVXWithBorrow(uint8_t x,
                                                            uint8_t y) {
    GET_VX_VY
...
void OpCodeExecutor::OPCODE_8XY7_SubtractVXFromVYWithBorrow(uint8_t x,
                                                            uint8_t y) {
    GET_VX_VY
...
void OpCodeExecutor::OPCODE_EX9E_SkipIfVXPressed(uint8_t x) {
    ...
    if (vx < 16) {
        ...
        std::cerr << "Warning: Invalid key index in OPCODE_EX9E: "
                  << static_cast<int>(vx) << std::endl;
    }
}
...
void OpCodeExecutor::OPCODE_FX1E_AddVXToI(uint8_t x) { CHIP->I += CHIP->V[x]; }
